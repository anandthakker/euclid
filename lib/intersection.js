...
class Intersection extends Point {
  ...
  toString(verbose) {
    ...
    return (!verbose) ? pstr :
      pstr + ' objects:' + this.objects.map(o => o.toString()).join(',');
  } 
}
...
function P(index, x, y, ...objects) {
  let name = 'Intersection(' + objects.map(o => o.name).join(',')+')['+index+']';
  return new Intersection(name, x, y, ...objects);
}
...
function intersectCircleCircle(c1, c2) {
  ...
  return uniq([P(0, cx+nx, cy-ny, c1, c2),
                 P(1, cx-nx, cy+ny, c1, c2)], comparePoints);
}
...
function intersectSegmentSegment(s1, s2) {
  ...
  if(s >= 0 && s <= 1 && t >= 0 && t <= 1)
    return uniq([P(0, x1 + t*s1.dx, y1 + t*s1.dy, s1, s2)], comparePoints);
  else
    return []; // no collision
}
...
function intersectCircleSegment(c, s) {
  ...
  // translate (0,0)->(x0, y0).
  return uniq([P(0, cx + nx + x0, cy + ny + y0, c, s), 
                 P(1, cx - nx + x0, cy - ny + y0, c, s)], comparePoints)
