...
function unique(points) { return _.uniq(points, false, p=>p.toString()); }
...
/* Return all intersections among given collection of objects */
function intersections(bounds, objects) {
  let finite = objects
  .filter(obj => !(obj instanceof Point))
  .map(obj => (obj instanceof Line) ? Segment.clip(bounds, obj) : obj)
  ...
  for(let i = 0; i < finite.length; i++) {
    for(let j = 0; j < i; j++) {
      ...
      points.forEach(function(p, k) {
        p._index = k; // TODO: Clean up this hack (it's for scene.js's intersection management)
