...
function intersectCircleCircle(c1, c2) {
  ...
  return uniq([P(cx+nx, cy-ny, c1, c2),
                 P(cx-nx, cy+ny, c1, c2)], comparePoints);
}
...
function intersectSegmentSegment(s1, s2) {
  ...
  if(s >= 0 && s <= 1 && t >= 0 && t <= 1)
    return uniq([P(x1 + t*s1.dx, y1 + t*s1.dy, s1, s2)], comparePoints);
  else
    return []; // no collision
}
...
function intersectCircleSegment(c, s) {
  ...
  // translate (0,0)->(x0, y0).
  return uniq([P(cx + nx + x0, cy + ny + y0, c, s), 
                 P(cx - nx + x0, cy - ny + y0, c, s)], comparePoints)
         
         .filter(p=>s.y(p.x)); // filter out points not defined on segment
}
